...
React Developer Tools는 React로 빌드된 웹 사이트를 디버깅할 수 있는 브라우저 확장 프로그램으로, Profiler를 사용하면 아래와 같은 작업이 가능합니다.
...
React DevTools의 Profiler를 통해 메시지 1개를 전송하는 과정에서 총 7개의 Commit이 발생한 것을 확인할 수 있었습니다. Commit은 React가 렌더링 결과를 실제 DOM에 반영하는 단계로, 사용자가 체감하는 화면 업데이트 비용과 직접적으로 연결됩니다.
...
이러한 문제를 해결하기 위해 아래 3가지의 해결 과정을 거쳤습니다.
...
이렇게 개별 메시지 렌더링은 `MessageList`로, 메시지 렌더링은 `MessageItem`으로 책임을 분리했습니다.
...
`MessageItem`은 props가 바뀌지 않으면 다시 렌더링 되지 않습니다. 따라서 새 메시지가 추가되더라도 기존 메시지 아이템은 렌더링 되지 않아 렌더링 비용을 줄일 수 있습니다.
...
메시지를 DB에서 가져오거나 새로 생성하는 시점에 `formattedTime`을 미리 계산하도록 변경했습니다. 이 변경으로 렌더링 중 시간 포맷 계산 비용을 제거할 수 있었습니다.
...
기존에는 메시지 1개를 전송할 때마다 `ChatRoom` 전체가 반복적으로 렌더링 되었고, 각 Commit마다 40\~60ms 수준의 렌더링 비용이 발생하고 있었습니다. 개선 후에는 대부분의 Commit이 1\~2ms 수준으로 줄어들었고, 마지막 병목이었던 `MessageList`의 렌더링 시간도 53.2ms에서 21.6ms로 약 60% 감소한 것을 확인할 수 있었습니다.
...
React DevTools를 사용해서 렌더링 속도를 직접 측정해 본 건 처음이었는데, 화면을 직접 녹화하고 그 과정에서 어떤 컴포넌트가 얼마나 빠르게 렌더링 되는지를 바로 보여줘서 굉장히 편리했습니다.

처음엔 '컴포넌트 분리 정도만 하면 되지 않을까?' 하는 생각으로 시작한 작업이었는데, 렌더링 과정에서 수행되는 작은 연산들이 반복되면 예상보다 큰 병목이 될 수 있다는 점을 느꼈습니다.

이번 작업을 통해 채팅방의 렌더링 성능을 대폭 개선했지만, 채팅방에 메시지가 쌓이면 쌓일수록 어쩔 수 없이 렌더링 비용이 증가하게 됩니다. 이런 경우, 메시지 리스트에 **가상화**를 적용해 렌더링 병목을 해소할 수 있습니다.

프론트엔드 가상화(Virtualization)는 데이터 중 화면에 실제로 보이는 영역의 요소만 렌더링하고, 나머지는 메모리에서 제외하여 브라우저의 연산 부담을 줄이는 성능 최적화 기법으로, 화면에 실제로 보이는 메시지만 렌더링함으로써 렌더링 성능을 향상시킬 수 있습니다.
